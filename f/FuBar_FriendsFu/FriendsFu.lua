...
FriendsFu = FuBarPlugin:GetInstance("1.2"):new({
	
	name          = FriendsFULocals.NAME,
	description   = FriendsFULocals.DESCRIPTION,
	version       = "1.51",
	...
	website       = "",
	category      = "interface",
	db            = AceDatabase:new("FriendsFUDB"),
	defaults      = DEFAULT_OPTIONS,
	cmd           = AceChatCmd:new(FriendsFULocals.COMMANDS, FriendsFULocals.CMD_OPTIONS),
	hasIcon       = TRUE,
	colorRank	  = "|cffff9933",
	colorNotes	  = "|cffff9933",
	...
	Initialize = function(self)
		self:Show()
		if GetNumFriends() == 0 then self:Hide() end
	end,
    ...
    prepare_data = function(self)
   	...
   	SystemMessage = function(self)
		if ( string.find(arg1, self.loc.SYSMSG_ONLINE) or string.find(arg1, self.loc.SYSMSG_OFFLINE) or string.find(arg1, self.loc.SYSMSG_ADDED) or string.find(arg1, self.loc.SYSMSG_REMOVED) ) then
			self:Update()
		end
	end,
		...
		for friendIndex=1, NumFriends do
			...
			if ( friend_connected ) then
				if ( self:passfilter({friend_name, friend_level,friend_class, friend_area} ) )then
					...
					table.insert(friendtable,{friend_name, friend_level,friend_class, friend_area})
				end
			end	
		end
