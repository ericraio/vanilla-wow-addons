...
local vmajor, vminor = "1", tonumber(string.sub("$Revision: 1705 $", 12, -3))
...
function lib:RegisterTooltip(tooltip)
	if self.vars.tooltip then return self.vars.tooltip end

	assert(tooltip, "No tooltip passed!")
	assert(tooltip:IsObjectType("GameTooltip"), "The frame passed was not a GameTooltip!")

	self.vars.tooltip = tooltip
...
--	tooltip:SetParent()
	...
	for i=1,30 do
		self.vars.Llines[i] = g[name.."TextLeft"..i]
		self.vars.Rlines[i] = g[name.."TextRight"..i]
	end
...
local babbleCore
--	If text is found on tooltip then results of babbleCore:Deformat are returned
--  Args:
...
function lib:FindDeformat(txt, startln, endln, ignoreleft, ignoreright)
	assert(txt, "No search string passed")
	if not babbleCore then
		assert(BabbleLib, "You must have BabbleLib-Core 1.1 available")
		babbleCore = BabbleLib:GetInstance("Core 1.1")
	end

	for i=(startln or 1),self:NumLines(endln) do
		if not ignoreleft then
			local txtl = self.vars.Llines[i]:GetText()
			if (txtl and babbleCore:Deformat(txtl, txt)) then return babbleCore:Deformat(txtl, txt) end
		end

		if not ignoreright then
			local txtr = self.vars.Rlines[i]:GetText()
			if (txtr and babbleCore:Deformat(txtr, txt)) then return babbleCore:Deformat(txtr, txt) end
		end
	end
end
...
function lib:CreateSetMethods()
	for _,m in pairs(methods) do
		...
		lib[meth] = function(self,a1,a2,a3,a4)
			...
			return self.vars.tooltip[meth](self.vars.tooltip,a1,a2,a3,a4)
		end
	end
end
