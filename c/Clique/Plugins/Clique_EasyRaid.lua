...
local Plugin = Clique:NewModule("easyraid")
Plugin.fullname = "Easy Raid"
Plugin.url = "http://www.wowinterface.com/downloads/fileinfo.php?id=4810"
...
function Plugin:OnEnable()
    self:Hook("ER_RaidPulloutButton_OnClick", "OnClick")
end
...
function Plugin:OnClick()
    ...
    if not Clique:OnClick(button, unit) then
        self:Debug("Calling the original function")
        self.hooks["ER_RaidPulloutButton_OnClick"].orig()
    end
end